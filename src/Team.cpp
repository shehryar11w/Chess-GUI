...
void Team::SetupPieces() {
    ...
    for (int i = 0; i < 6; i++) {
        ...
        texManager->LoadTextureFromFile(key, filepath);
    }
...
void Team::AddPiece(int x, int y, char type) {
    ...
    switch (upperType) {
        case 'P': texKey = color + "_pawn"; break;
        case 'R': texKey = color + "_rook"; break;
        case 'N': texKey = color + "_knight"; break;
        case 'B': texKey = color + "_bishop"; break;
        case 'Q': texKey = color + "_queen"; break;
        case 'K': texKey = color + "_king"; break;
        default: return;
    }
    
    Texture2D tex = texManager->GetTexture(texKey);
    if (tex.id == 0) return;  // Skip if texture not found
